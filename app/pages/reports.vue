                ...
                <div
                  ...
                  :style="`width: ${
                    dailyReport.total_amount > 0
                      ? (dailyReport.onlineAmount / dailyReport.total_amount) *
                        100
                      : 0
                  }%`"
                ></div>
            ...
            <div>
              ...
              <div class="w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  :style="`width: ${
                    dailyReport.total_amount > 0
                      ? (dailyReport.counterAmount / dailyReport.total_amount) *
                        100
                      : 0
                  }%`"
                ></div>
...
const dailyReportRaw = await getDailyReport(today);
const monthlyReportRaw = await getMonthlyReport(currentYear, currentMonth);

// Calculate sales type breakdown for daily
const dailySales = await getSales({
  sale_date: {
...
const dailyOnlineSales = dailySales.filter((s) => s.sale_type === "online");
const dailyCounterSales = dailySales.filter((s) => s.sale_type === "counter");
...
const monthlySales = monthlyReportRaw.items;
const monthlyOnlineSales = monthlySales.filter((s) => s.sale_type === "online");
const monthlyCounterSales = monthlySales.filter(
  (s) => s.sale_type === "counter"
);

// Calculate profit for top products
const topProductsWithProfit = monthlyReportRaw.topProducts.map((item) => {
  const productId =
  ...
  const profit = productSales.reduce((sum, si) => sum + (si.profit || 0), 0);
